...
<body
			...
			<div id="MainMenu">
				<div class="list-group panel">
					<a href="#demo5" class="list-group-item list-group-item strong"
						data-toggle="collapse" data-parent="#MainMenu"><i
						class="fa fa-cubes"></i> Categories <i class="fa fa-caret-down"></i></a>
					<div class="collapse list-group-submenu" id="demo5">
						<div id="catdisplay" class="list-group-item"
							onClick="searchCategoryWithLevel()"></div>
					</div>
				</div>
			</div>
		</div>
	...
	<div class="container1">
		...
		<button class="tablink"
			onClick="loadQuestions(); return loadCategories();">MCQ</button>
		<button class="tablink"
			onClick="loadCodingQuestions(); return loadCodingCategories();"
...
<script>
	...
	function searchCategoryWithLevel() {
		console.log("searching question based on category");
		...
		if (document.querySelector('input[name="cat"]:checked').value != "null") {
			var category = document.querySelector('input[name="cat"]:checked').value;
			category = category.concat("_");
			category = category.concat(lValue);
	...
	function loadQuestions() {
		...
		$('#failure_p').hide();
		$('#success_p').hide();
		temp = sessionStorage.getItem("level");
		console.log(temp);
										...
										.getJSON(
												...
												function(json) {
													...
													for (var i = 0; i < json.length; i++) {
														if (json[i].type == "mcq") {
															tr
																	.append("<a class='list-group-item'><input type='radio' name='cat' value='" + json[i].categoryname + "'>"
																			+ json[i].categoryname
																			+ "</a>");
														}
													}
